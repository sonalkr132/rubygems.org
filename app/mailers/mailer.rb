class Mailer < ActionMailer::Base
  ...
  def adoption_application_applied(adoption_application)
    adoption_application = AdoptionApplication.find(adoption_application['id'])
    adoption_application.rubygem.owners.each do |owner|
      mail from: Clearance.configuration.mailer_sender,
           to: owner.email,
           subject: I18n.t('mailer.adoption_application_applied.subject', gem: adoption_application.rubygem.name) do |format|
        format.html { render locals: { adoption_application: adoption_application, owner: owner } }
      end
    end
  end

  def adoption_application_closed(rubygem, user)
    user = User.find(user['id'])
    rubygem = Rubygem.find(rubygem['id'])

    mail from: Clearance.configuration.mailer_sender,
         to: user.email,
         subject: I18n.t('mailer.adoption_application_closed.subject', gem: rubygem.name) do |format|
